document.addEventListener("DOMContentLoaded", () => {
  ...
  function formatItunesStoreUrl(url) {
    ...
    let clean = url.trim();
    if (!clean || clean.startsWith("javascript:") || clean.includes("artist/henna-c") || clean.includes("distrokid.com") || clean.includes("hyperfollow")) return clean;
  ...
  const RECENT_SINGLES_CATALOG = [
    {
      ...
      genre: "Hip-Hop / Pop",
      meta: "Hip-Hop / Pop · Out Now",
      kicker: "New Single · Out Now",
      badge: "LATEST SINGLE",
      art: "assets/pressure-makes-diamonds.png",
      src: "assets/audio/pressure-makes-diamonds.wav",
      itunes: "",
    ...
    },
    {
      ...
      kicker: "New Single · Out Now",
      badge: "LATEST SINGLE",
      art: "assets/after-hours.png",
      src: "assets/audio/after-hours.wav",
      itunes: "https://distrokid.com/hyperfollow/hennac/after-hours",
  ...
  function renderRecentSingles() {
    ...
    miniGrid.innerHTML = topSingles.map(single => {
      const isHyperfollow = Boolean(single.itunes && (single.itunes.includes("distrokid.com") || single.itunes.includes("hyperfollow")));
      const itunesLink = isHyperfollow ? single.itunes : formatItunesStoreUrl(single.itunes);
      const isAvail = Boolean(itunesLink && !itunesLink.includes("artist/henna-c"));
      const btnLabel = isHyperfollow ? "Stream / Save" : (isAvail ? "Purchase on iTunes" : "Listen Now");
      const btnClass = isHyperfollow ? "mini-itunes-btn" : (isAvail ? "mini-itunes-btn" : "mini-itunes-btn pl-stream-btn");
      const btnIcon = (isHyperfollow || !isAvail) ? `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14.5v-9l6 4.5-6 4.5z"/></svg>` : `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 4.17c.66-.81 1.11-1.93.99-3.06-1 .04-2.2.67-2.92 1.5-.63.73-1.18 1.87-1.03 2.98 1.12.09 2.27-.61 2.96-1.42"/></svg>`;
      ...
      <div class="mini-card visible" id="mini-card-${single.id}">
        ...
        <div class="mini-text">
          ...
          <div class="mini-actions">
            ...
            ${isAvail ? `
            <a href="${itunesLink}" target="_blank" rel="noopener noreferrer" class="${btnClass}" title="${isHyperfollow ? `Stream ${single.title}` : `Purchase ${single.title} on iTunes`}">
              ${btnIcon}
              <span>${btnLabel}</span>
            </a>` : `
            <button type="button" class="${btnClass}" data-title="${single.title}" title="Listen to ${single.title}">
              ${btnIcon}
              <span>${btnLabel}</span>
            </button>`}
          </div>
    ...
    // Bind playback, dropdown, and stream listeners to mini cards
    ...
    miniGrid.querySelectorAll(".pl-stream-btn").forEach(btn => {
      btn.addEventListener("click", e => {
        e.stopPropagation();
        toggleDropdown(btn, e, btn.dataset.title);
      });
    });
